...
void checkAnswer() {
    ...
    if ((numbers[0] == currentProblem.a && numbers[1] == currentProblem.b)|| 
        (numbers[0] == currentProblem.b && numbers[1] == currentProblem.a)) {
        score++;
        if (score % 5 == 0) {
            level++;
            speed += 0.5;
...
void updateOpponentState(JSONObject state) {
    opponentState = state;
}
...
void sendGameState() {
    ...
    state.setInt("level", level);
    
    // JavaScriptの関数を呼び出してWebSocketで送信
    javascript.postGameState(state.toString());
}

void drawOpponentInfo() {
    if (opponentState != null && isOnline) {
        fill(50, 50, 100);
        textAlign(RIGHT, TOP);
        textSize(20);
        text("対戦相手のスコア: " + opponentState.getInt("score"), width - 10, 10);
    }
}
